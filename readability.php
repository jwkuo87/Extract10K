...
// Check to see if output file already exist
if (file_exists($outputdir. "output.txt"))
    {
    file_put_contents($outputdir. "output.txt" , "-;-;-;-;-;-;-;-\r\n", FILE_APPEND | LOCK_EX);
    }
else
    {
...
if (file_exists($outputdir. "log.txt"))
    ...
    {
    $log = array();
    file_put_contents($outputdir. "log.txt" , "File" . "\r\n");
    }
        ...
        while (($file = readdir($dh)) !== false)
            ...
            if (filetype($dir . $file) == 'file')
                ...
                if (!in_array($file."\r\n", $log))
                    ...
                    // Define the output for the analysis
                    $output = 
                    $file
                    . ";" . $statistics->gunning_fog_score($text)
                    //. ";" . $statistics->flesch_kincaid_reading_ease($text)
                    . ";" . $statistics->flesch_kincaid_grade_level($text)
                    //. ";" . $statistics->coleman_liau_index($text)
                    //. ";" . $statistics->automated_readability_index($text)
                    . ";" . $statistics->sentence_count($text)
                    . ";" . $statistics->word_count($text)
                    . ";" . $statistics->unique_word_count($text)
                    . ";" . $statistics->total_syllables($text)
                    . ";" . $statistics->letter_count($text)
                    . "\r\n";
                
                    // Define the output for the logfile
                    $logoutput = 
                    $file
                    . "\r\n";
...
class TextStatistics
{
    ...
    protected function clean_text($strText)
    {
        ...
        $strText = preg_replace('`16.9;29.2;12;12;12;10;186;349;977<script(.*?)>(.*?)</script>`is', '', $strText);
        ...
        // lines with spaces
        // Use option 1 if line breaks in text are not end of sentences and option 2 if they are
        //
        // Option 1:
     ...
     * @return  int
     */
    public function unique_word_count($strText)
    {
        $strText = $this->clean_text($strText);
        $intUniqueWordCount = 0;
        $arrWords = explode(' ', strtolower(preg_replace('`[^A-za-z\' ]`', '', $strText)));
